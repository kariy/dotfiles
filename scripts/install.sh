...
# Usage: select_components "prompt" "${options[@]}"
select_components() {
  if $AUTO_YES; then
    # In non-interactive mode, select everything
    SELECTED=()
    local i
...
Examples:
  curl -fsSL https://example.com/install.sh | bash              # interactive
  curl -fsSL https://example.com/install.sh | bash -s -- -y     # install all
EOF
  exit 0
}
...
# Maps component display names to make targets
COMPONENTS=(
